...
def camConnect(timeout = 2.0):
    ...
    try:
        response = requests.get(
            f"{API_BASE}api/v2/streams/mjpeg", timeout=timeout)
...
def disconnect():
    ...
    prints conformation if (success code) returns; raises error if not"""
    try:
        response = requests.get(f"{API_BASE}api/v2/actions/system/shutdown/", timeout=2.0) #path extention for disconnecting from the api & microscope
